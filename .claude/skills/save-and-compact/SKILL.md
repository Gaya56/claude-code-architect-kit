...
7. TELL the user: "State saved. Here's what I captured: [brief summary]"
8. TELL the user: "Ready to compact. Run /compact now, and when you're back,
...
**Context7 Integration:** The `context7` plugin is enabled globally. Use it to systematically gather all dynamic context (git status, branch, recent commits, uncommitted changes, active tasks, decisions, blockers) so nothing is lost during compaction. Context7 helps ensure complete state capture before the session breaks.